...
const CourseEditorFields: React.FC<{ value: CourseForm; onChange: (value: CourseForm) => void }> = ({ value, onChange }) => (
  <div className="pf-course-editor">
    ...
    <label className="pf-admin-field"><span>Status</span><select className="pf-admin-select" value={value.status} onChange={(event) => onChange({ ...value, status: event.target.value as CourseForm['status'] })}><option value="ACTIVE">Active</option><option value="INACTIVE">Inactive</option><option value="ARCHIVED">Archived</option></select></label>
  </div>
);
...
export const AcademyCoursesReadOnlyPage: React.FC = () => {
  ...
  return <motion.div className="pf-admin-page pf-academy-courses" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.24 }}>
    ...
    <section className="pf-admin-table-card">
      <div className="pf-admin-table-card__header pf-academy-course-toolbar"><label className="pf-admin-search-field"><Search size={17} /><span className="pf-admin-sr-only">Search Academy courses</span><input className="pf-admin-input" value={search} onChange={(event) => { setSearch(event.target.value); setPage(1); }} placeholder="Search courses by name, code, or description…" /></label><select className="pf-admin-select" value={status} onChange={(event) => { setStatus(event.target.value as CourseStatus | ''); setPage(1); }}><option value="">All Statuses</option><option value="ACTIVE">Active</option><option value="INACTIVE">Inactive</option><option value="ARCHIVED">Archived</option></select></div>

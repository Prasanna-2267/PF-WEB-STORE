    ...
    <div className="pf-question-workspace__body">
      <div className="pf-question-editor-main">
        <section className="pf-question-form-card"><header><span>01</span><div><h3>Question content</h3><p>Build the learner-facing content and answer logic.</p></div></header>
          {question.kind.startsWith('CASE_') ? <><RichTextEditor label="Case passage" value={question.caseHtml} onChange={(caseHtml) => setQuestion({ ...question, caseHtml })} error={errors.caseHtml} placeholder="Write the case, facts, or shared scenario…" /><div className="pf-question-editor__row"><label className="pf-admin-field"><span>Case ID</span><input className="pf-admin-input" value={question.caseId ?? ''} onChange={(event) => setQuestion({ ...question, caseId: event.target.value })} placeholder="CASE-AUD-001" /></label><label className="pf-admin-field"><span>Classification</span><AppSelect className="pf-admin-select" value={question.classificationMode} onChange={(event) => setQuestion({ ...question, classificationMode: event.target.value as QuestionRecord['classificationMode'] })}><option value="ENTIRE_CASE">Entire case</option><option value="INDIVIDUAL_SUB_QUESTIONS">Each sub-question</option></AppSelect></label></div></> : <RichTextEditor label="Question" value={question.questionHtml} onChange={(questionHtml) => setQuestion({ ...question, questionHtml })} error={errors.questionHtml} placeholder="Write the question…" />}
      ...
      <aside className="pf-question-editor-side"><section className="pf-question-form-card"><header><span>{question.kind.startsWith('CASE_') ? '03' : '02'}</span><div><h3>Classification</h3><p>Use the existing academic hierarchy.</p></div></header>{question.classificationMode === 'ENTIRE_CASE' || !question.kind.startsWith('CASE_') ? <TaxonomySelector taxonomy={taxonomy} value={question.classification} onChange={(classification) => setQuestion({ ...question, classification })} errors={errors} /> : <p className="pf-question-muted">Classification is configured inside each sub-question.</p>}</section><section className="pf-question-form-card"><label className="pf-admin-field"><span>Difficulty</span><AppSelect className="pf-admin-select" value={question.difficulty} onChange={(event) => setQuestion({ ...question, difficulty: event.target.value as QuestionRecord['difficulty'] })}><option value="FOUNDATION">Foundation</option><option value="INTERMEDIATE">Intermediate</option><option value="ADVANCED">Advanced</option></AppSelect></label><p className="pf-question-autosave"><Save size={15} /> Temporary editor state is preserved in this browser.</p></section></aside>
    </div>

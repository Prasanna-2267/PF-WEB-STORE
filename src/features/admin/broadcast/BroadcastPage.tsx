...
const emptyForm = (scope: 'platform' | 'academy' = 'platform'): BroadcastForm => ({
  ...
  platform: 'BOTH', placements: ['NOTIFICATION'], startAt: null, endAt: null,
...
const BasicStep: React.FC<StepProps> = ({ form, patch, errors = {}, scope = 'platform' }) => <>
  ...
  <div className="pf-broadcast-form-grid">
    ...
    <label className="pf-broadcast-field"><span>Broadcast type *</span><select className="pf-admin-select" value={form.type} onChange={(event) => patch('type', event.target.value as BroadcastType)}>{Object.entries(BROADCAST_TYPE_LABELS).filter(([value]) => scope === 'platform' || !['PROMOTION', 'STORE'].includes(value)).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
...
const DisplayStep: React.FC<DisplayStepProps> = ({ form, patch, changePlatform, togglePlacement, error, scope = 'platform' }) => <>
  ...
  <label className="pf-broadcast-field"><span>Presentation style *</span><select className="pf-admin-select" value={form.presentation} onChange={(event) => patch('presentation', event.target.value as BroadcastPresentation)}>{Object.entries(BROADCAST_PRESENTATION_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
  <fieldset className="pf-broadcast-fieldset"><legend>Platform</legend><div className="pf-broadcast-segments">{(['APP', 'WEBSITE', 'BOTH'] as BroadcastPlatform[]).map((item) => <button type="button" key={item} className={form.platform === item ? 'is-active' : ''} onClick={() => changePlatform(item)}>{item === 'BOTH' ? 'App & website' : item === 'APP' ? 'App' : 'Website'}</button>)}</div></fieldset>
  ...
  <label className="pf-broadcast-field"><span>Priority</span><select className="pf-admin-select" value={form.priority} onChange={(event) => patch('priority', event.target.value as BroadcastPriority)}><option value="LOW">Low — minor information</option><option value="NORMAL">Normal — general update</option><option value="HIGH">High — important announcement</option><option value="CRITICAL">Critical — emergency or maintenance</option></select></label>
...
const BehaviorStep: React.FC<StepProps> = ({ form, patch }) => <>
  ...
  <div className="pf-broadcast-form-grid">
    <label className="pf-broadcast-field"><span>Display frequency</span><select className="pf-admin-select" value={form.frequency} onChange={(event) => patch('frequency', event.target.value as BroadcastFrequency)}>{Object.entries(BROADCAST_FREQUENCY_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select><small>{form.frequency === 'ONCE' ? 'Shown once to each eligible user.' : form.frequency === 'DAILY' ? 'May appear once per calendar day.' : form.frequency === 'EVERY_VISIT' ? 'Appears at each eligible session.' : form.frequency === 'UNTIL_DISMISSED' ? 'Repeats until the user dismisses it.' : 'Remains visible for the entire active period.'}</small></label>
    <label className="pf-broadcast-field"><span>Repeat behavior</span><select className="pf-admin-select" value={form.repeatBehavior} onChange={(event) => patch('repeatBehavior', event.target.value as BroadcastRepeatBehavior)}>{Object.entries(BROADCAST_REPEAT_BEHAVIOR_LABELS).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
  </div>
    ...
    {store.status === 'error' ? <AdminEmptyState title="Broadcasts could not be loaded" description={store.error ?? 'Try loading the workspace again.'} icon={<ShieldAlert size={22} />} action={<button className="pf-admin-button" onClick={() => void store.refresh()}><RefreshCw size={15} /> Try again</button>} /> : <>
      ...
      {broadcasts.length === 0 ? <AdminEmptyState title="No broadcasts yet" description={scope === 'academy' ? 'Create the first announcement for this Academy. No fixture broadcasts are displayed.' : 'Create your first announcement, update, promotion or important notice.'} icon={<Megaphone size={22} />} action={<button className="pf-admin-button" onClick={() => setEditor(null)}><Plus size={15} /> New broadcast</button>} /> : <section className="pf-broadcast-library">
        <div className="pf-broadcast-toolbar">
          ...
          <label><span>Status</span><select className="pf-admin-select" value={statusFilter} onChange={(event) => setStatusFilter(event.target.value as StatusFilter)}><option value="CURRENT">Current (excludes archived)</option><option value="ALL">All statuses</option><option value="ACTIVE">Active</option><option value="SCHEDULED">Scheduled</option><option value="DRAFT">Draft</option><option value="DISABLED">Disabled</option><option value="EXPIRED">Expired</option><option value="ARCHIVED">Archived</option></select></label>
          <label><span>Type</span><select className="pf-admin-select" value={typeFilter} onChange={(event) => setTypeFilter(event.target.value as 'ALL' | BroadcastType)}><option value="ALL">All types</option>{Object.entries(BROADCAST_TYPE_LABELS).filter(([value]) => scope === 'platform' || !['PROMOTION', 'STORE'].includes(value)).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label>
          <label><span>Audience</span><select className="pf-admin-select" value={audienceFilter} onChange={(event) => setAudienceFilter(event.target.value as 'ALL' | BroadcastAudienceKind)}><option value="ALL">All audiences</option>{Object.entries(BROADCAST_AUDIENCE_LABELS).filter(([value]) => scope === 'platform' || ['ACADEMY_STUDENTS', 'COURSES'].includes(value)).map(([value, label]) => <option key={value} value={value}>{scope === 'academy' && value === 'ACADEMY_STUDENTS' ? 'All Academy students' : label}</option>)}</select></label>
          <label><span>Priority</span><select className="pf-admin-select" value={priorityFilter} onChange={(event) => setPriorityFilter(event.target.value as 'ALL' | BroadcastPriority)}><option value="ALL">All priorities</option><option value="CRITICAL">Critical</option><option value="HIGH">High</option><option value="NORMAL">Normal</option><option value="LOW">Low</option></select></label>
          <label><span>Date</span><select className="pf-admin-select" value={dateFilter} onChange={(event) => setDateFilter(event.target.value as DateFilter)}><option value="ANY">Any date</option><option value="TODAY">Today</option><option value="NEXT_7">Next 7 days</option><option value="LAST_30">Past 30 days</option></select></label>
          <label><span>Sort</span><select className="pf-admin-select" value={sort} onChange={(event) => setSort(event.target.value as SortOption)}><option value="NEWEST">Newest</option><option value="OLDEST">Oldest</option><option value="MODIFIED">Recently modified</option><option value="START">Start date</option><option value="END">End date</option><option value="PRIORITY">Priority</option></select></label>
        </div>
        {visible.length ? <>

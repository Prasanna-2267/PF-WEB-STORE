...
adminRouter.patch("/students/:userId/status", requirePermission("students:manage"), asyncRoute(async (req, res) => { const body = z.object({ status: z.enum(["ACTIVE", "DISABLED"]) }).strict().parse(req.body); res.json(await adminService.updateAdminUserStatus(req.auth!.userId, id.parse(req.params.userId), body.status)); }));
